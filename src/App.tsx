...
function ProviderDialog({ providers, onClose, onChanged }: { providers: Provider[]; onClose: () => void; onChanged: () => Promise<void> }) {
  const [name, setName] = useState(""); const [kind, setKind] = useState<ProviderKind>("openai_compatible"); const [baseUrl, setBaseUrl] = useState("https://api.openai.com/v1"); const [key, setKey] = useState(""); const [error, setError] = useState<string | null>(null); const [busy, setBusy] = useState(false); const [editingId, setEditingId] = useState<string | null>(providers[0]?.id ?? null);
  function setProviderKind(next: ProviderKind) { setKind(next); setBaseUrl((current) => current === "https://api.openai.com/v1" || current === "https://api.anthropic.com" ? (next === "anthropic" ? "https://api.anthropic.com" : "https://api.openai.com/v1") : current); }
  async function submit(event: FormEvent) { event.preventDefault(); setBusy(true); setError(null); try { await api.createProvider({ name, kind, base_url: baseUrl, api_key: key }); await onChanged(); setName(""); setKey(""); } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to save provider."); } finally { setBusy(false); } }
