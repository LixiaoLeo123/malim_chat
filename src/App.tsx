...
function Chat() {
  ...
  const [generation, setGeneration] = useState({ temperature: 0.7, reasoning_effort: "medium", enable_markdown: true });
...
function GenerationButton({ generation, onChange, provider, model }: { generation: { temperature: number; reasoning_effort: string; enable_markdown: boolean }; onChange: (value: { temperature: number; reasoning_effort: string; enable_markdown: boolean }) => void; provider?: Provider; model: string }) {
...
function DictionaryPopover({ word, anchor, onClose }: { word: string; anchor: { x: number; y: number }; onClose: () => void }) {
  ...
  useEffect(() => {
    const place = () => {
      ...
      const useUp = anchor.y + 8 + rect.height > window.innerHeight - margin && anchor.y - rect.height - 18 >= margin;
      const top = useUp ? anchor.y - rect.height - 18 : Math.max(margin, Math.min(anchor.y + 8, window.innerHeight - rect.height - margin));
      setPlacement(useUp ? "up" : "down"); setPosition({ left, top });
    };
