...
function Chat() {
  ...
  async function deleteConversation(id: string) {
    const before = useAppStore.getState().conversations;
    state.setConversations(before.filter((item) => item.id !== id));
    state.setMessages(id, []);
    if (activeId === id) state.setActiveId(before.find((item) => item.id !== id)?.id ?? null);
    try { await api.deleteConversation(id); }
    catch (cause) { state.setConversations(before); state.setError(cause instanceof Error ? cause.message : "Could not delete conversation."); }
  }
...
function Sidebar({ conversations, activeId, open, collapsed, onNew, onSelect, onRename, onDelete, onToggleFavorite, onSettings, onClose, onCollapse, userName, onSignOut }: { conversations: Conversation[]; activeId: string | null; open: boolean; collapsed: boolean; onNew: () => void; onSelect: (id: string) => void; onRename: (id: string, title: string) => Promise<void>; onDelete: (id: string) => Promise<void>; onToggleFavorite: (id: string) => void; onSettings: () => void; onClose: () => void; onCollapse: () => void; userName: string; onSignOut: () => void }) {
  ...
  useEffect(() => { if (confirmDeleteId && !conversations.some((conversation) => conversation.id === confirmDeleteId)) setConfirmDeleteId(null); }, [conversations, confirmDeleteId]);
  ...
  function disarmDelete() { if (confirmTimerRef.current) window.clearTimeout(confirmTimerRef.current); setConfirmDeleteId(null); }
  function armDelete(id: string) { disarmDelete(); setConfirmDeleteId(id); confirmTimerRef.current = window.setTimeout(() => setConfirmDeleteId(null), 3000); }
  function row(conversation: Conversation) { return <div className={`conversation-row ${conversation.id === activeId ? "selected" : ""}`} key={conversation.id} data-flip-id={conversation.id}>{renamingId === conversation.id ? <form className="conversation-rename" onSubmit={(event) => { event.preventDefault(); void finishRename(conversation.id); }}><input autoFocus value={draft} maxLength={160} onChange={(event) => setDraft(event.target.value)} onBlur={() => void finishRename(conversation.id)} /></form> : <button type="button" onClick={() => { disarmDelete(); onSelect(conversation.id); }}><MessageSquare size={16} /><span>{conversation.title}</span></button>}<button type="button" className={`row-action star ${conversation.is_favorite ? "active" : ""}`} aria-label={conversation.is_favorite ? "Remove from favorites" : "Add to favorites"} title={conversation.is_favorite ? "Remove from favorites" : "Add to favorites"} onMouseDown={(event) => event.preventDefault()} onClick={() => { disarmDelete(); onToggleFavorite(conversation.id); }}><Star size={15} fill={conversation.is_favorite ? "currentColor" : "none"} /></button><button type="button" className="row-action" aria-label={`Rename ${conversation.title}`} title="Rename conversation" onMouseDown={(event) => event.preventDefault()} onClick={() => { disarmDelete(); startRename(conversation); }}><Edit3 size={15} /></button><button type="button" className={`row-action delete${confirmDeleteId === conversation.id ? " armed" : ""}`} aria-label={confirmDeleteId === conversation.id ? "Confirm delete" : "Delete conversation"} title={confirmDeleteId === conversation.id ? "Click again to delete" : "Delete conversation"} onMouseDown={(event) => event.preventDefault()} onClick={() => { if (confirmDeleteId === conversation.id) { disarmDelete(); void onDelete(conversation.id); } else { armDelete(conversation.id); } }}>{confirmDeleteId === conversation.id ? <><Trash2 size={14} /><span className="delete-label">Delete?</span></> : <Trash2 size={15} />}</button></div>; }
...
function ProviderEditForm({ provider, onChanged, onError }: { provider: Provider; onChanged: () => Promise<void>; onError: (value: string | null) => void }) {
  const [name, setName] = useState(provider.name); const [baseUrl, setBaseUrl] = useState(provider.base_url); const [key, setKey] = useState(""); const [busy, setBusy] = useState(false);
  async function save(event: FormEvent) { event.preventDefault(); setBusy(true); onError(null); try { await api.updateProvider(provider.id, { name: name.trim() || undefined, base_url: baseUrl.trim() || undefined, api_key: key.trim() || undefined }); setKey(""); await onChanged(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Could not update provider."); } finally { setBusy(false); } }
  return <form className="provider-form" onSubmit={save}><h3>Edit provider</h3><label>Name<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Base URL<input type="url" value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></label><label>API key<input type="password" value={key} onChange={(event) => setKey(event.target.value)} placeholder="Leave blank to keep the current key" /></label><button className="primary" disabled={busy}>{busy && <LoaderCircle className="spin" size={17} />}Save changes</button></form>;
}
