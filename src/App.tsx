...
/**
 * Android raises its selection toolbar (Copy / Share / Define) from the same long press that
 * opens the dictionary popover, so the two fight. The packaged app finds the word through
 * `caretRangeFromPoint`, which keeps working without a native selection, so the app turns
 * native selection off and a browser does not.
 */
const androidApp = /android/i.test(navigator.userAgent) && "__TAURI_INTERNALS__" in window;
...
function Chat() {
  ...
  useEffect(() => {
    ...
    if (conversation) {
      const loaded = conversation.generation_settings ?? defaultGeneration;
      // A stored 0 is below the slider's floor of one round; clamp it so the value never strands
      // the thumb at 1 while the label reads 0.
      setGeneration(loaded.context_rounds === 0 ? { ...loaded, context_rounds: 1 } : loaded);
    }
  }, [activeId]);
