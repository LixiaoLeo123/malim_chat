...
function GenerationButton({ generation, onChange, kind, model }: { generation: GenerationSettings; onChange: (value: GenerationSettings) => void; kind?: ProviderKind; model: string }) {
  ...
  // Responses runs search and reasoning on the provider side: no transcript window to trim
  // and no ReAct loop to bound, so those two sliders do not apply.
...
type ModelDraft = { key: string; id?: string; group_name: string; model: string; kind: ProviderKind; context_window: number; supports_images: boolean; chain_context: boolean };
...
function ProviderSettings({ provider, onChanged, onError }: { provider: Provider; onChanged: () => Promise<void>; onError: (value: string | null) => void }) {
  ...
  const [adding, setAdding] = useState({ group_name: "General", model: "", kind: provider.kind, context_window: 128000, supports_images: false, chain_context: true });
  const [busy, setBusy] = useState(false);
  ...
  const modelChanged = (item: ModelDraft) => {
    ...
    return !before || before.group_name !== item.group_name || before.model !== item.model || before.kind !== item.kind || before.context_window !== item.context_window || before.supports_images !== item.supports_images || before.chain_context !== item.chain_context;
  };
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      for (const item of drafts) {
        const payload = { group_name: item.group_name.trim() || "General", model: item.model.trim(), kind: item.kind, context_window: item.context_window, supports_images: item.supports_images, chain_context: item.chain_context };
