...
function Chat() {
  ...
  async function sendMessage(conversationId: string, content: string, search: boolean, options = generation) {
    ...
    try {
      ...
      const answer = options.stream ? await api.respondStream(conversationId, sent.id, search, options, (delta) => { const current = useAppStore.getState().messages[conversationId]?.find((item) => item.id === pendingId); if (current) state.upsertMessage(conversationId, { ...current, content: current.content + delta }); }) : await api.respond(conversationId, sent.id, search, options);
  ...
  async function retryMessage(message: Message) {
    ...
    state.setBusy(true);
    try {
      const answer = generation.stream ? await api.respondStream(activeId, message.id, searchEnabled, generation, (delta) => { const current = useAppStore.getState().messages[activeId]?.find((item) => item.id === pendingId); if (current) state.upsertMessage(activeId, { ...current, content: current.content + delta }); }) : await api.respond(activeId, message.id, searchEnabled, generation);
...
function MessageBubble({ message, markdownEnabled, onEdit, onDelete, onRetry, onLookup }: { message: Message; markdownEnabled: boolean; onEdit: (message: Message, content: string) => Promise<void>; onDelete: (message: Message) => Promise<void>; onRetry: (message: Message) => Promise<void>; onLookup: (word: string, anchor: { x: number; y: number }) => void }) {
