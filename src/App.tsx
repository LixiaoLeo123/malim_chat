...
const defaultGeneration: GenerationSettings = { temperature: 0.7, reasoning_effort: "medium", enable_markdown: true, stream: true, builtin_tools: false, context_rounds: 8, tool_rounds: 4 };
function uuid() {
...
function Chat() {
  ...
  async function sendMessage(conversationId: string, content: string, search: boolean, options = generation, images: string[] = []) {
    ...
    try {
      ...
      const answer = options.stream ? await api.respondStream(conversationId, sent.id, search, options, (event) => { if (event.type === "tool") toolEvents = updateToolEvents(toolEvents, event.tool); else if (event.type === "reasoning") { streamedReasoning += event.delta ?? ""; toolEvents = updateToolEvents(toolEvents, { id: `reasoning-${toolEvents.length}-${event.round ?? 0}`, name: "reasoning", status: "completed", round: event.round, detail: event.delta }); } else streamedContent += event.delta ?? ""; state.upsertMessage(conversationId, { ...streamingBase, id: streamingId, sequence: streamingSequence, client_mutation_id: null, role: "assistant", content: streamedContent, images: [], reasoning_content: streamedReasoning, status: "streaming", model: null, token_count: 0, search_sources: [], tool_events: toolEvents }); }, controller?.signal) : await api.respond(conversationId, sent.id, search, options);
  ...
  async function retryMessage(message: Message) {
    ...
    state.setBusy(true);
    try {
      const answer = generation.stream ? await api.respondStream(activeId, target.id, searchEnabled, generation, (event) => { if (event.type === "tool") toolEvents = updateToolEvents(toolEvents, event.tool); else if (event.type === "reasoning") { streamedReasoning += event.delta ?? ""; toolEvents = updateToolEvents(toolEvents, { id: `reasoning-${toolEvents.length}-${event.round ?? 0}`, name: "reasoning", status: "completed", round: event.round, detail: event.delta }); } else streamedContent += event.delta ?? ""; state.upsertMessage(activeId, { ...target, id: pendingId, sequence: target.sequence + 0.5, client_mutation_id: null, role: "assistant", content: streamedContent, images: [], reasoning_content: streamedReasoning, status: "streaming", model: null, token_count: 0, search_sources: [], tool_events: toolEvents }); }, controller?.signal) : await api.respond(activeId, target.id, searchEnabled, generation);
...
function GenerationButton({ generation, onChange, kind, model }: { generation: GenerationSettings; onChange: (value: GenerationSettings) => void; kind?: ProviderKind; model: string }) {
  ...
  const context = generation.context_rounds ?? 8; const tools = generation.tool_rounds ?? 4;
  return <div ref={controlRef} className="generation-control"><button type="button" className="search-chip" onClick={() => setOpen(!open)} title="Generation parameters"><Settings2 size={14} />Parameters</button>{open && <div className="generation-popover"><label className="markdown-toggle"><span>OpenAI native tools</span><input type="checkbox" checked={generation.builtin_tools ?? false} onChange={(event) => onChange({ ...generation, builtin_tools: event.target.checked })} /></label><label><span>Temperature <b>{generation.temperature.toFixed(1)}</b></span><input type="range" min="0" max="2" step="0.1" value={generation.temperature} onChange={(event) => onChange({ ...generation, temperature: Number(event.target.value) })} /></label>{supportsReasoning && <label><span>Reasoning effort <b>{generation.reasoning_effort}</b></span><input type="range" min="0" max="2" step="1" value={effort.indexOf(generation.reasoning_effort as typeof effort[number])} onChange={(event) => onChange({ ...generation, reasoning_effort: effort[Number(event.target.value)] })} /><small>Low <i /> Medium <i /> High</small></label>}<label><span>Recent context rounds <b>{generation.context_rounds === null ? "All" : context}</b></span><input type="range" min="0" max="20" step="1" value={generation.context_rounds === null ? 20 : context} onChange={(event) => onChange({ ...generation, context_rounds: Number(event.target.value) })} /><small>0 <i /> 20 <button type="button" className="inline-option" onClick={() => onChange({ ...generation, context_rounds: null })}>All</button></small></label><label><span>Maximum ReAct rounds <b>{generation.tool_rounds === null ? "Unlimited" : tools}</b></span><input type="range" min="1" max="20" step="1" value={generation.tool_rounds === null ? 20 : tools} onChange={(event) => onChange({ ...generation, tool_rounds: Number(event.target.value) })} /><small>1 <i /> 20 <button type="button" className="inline-option" onClick={() => onChange({ ...generation, tool_rounds: null })}>Unlimited</button></small></label><label className="markdown-toggle"><span>Enable Markdown</span><input type="checkbox" checked={generation.enable_markdown} onChange={(event) => onChange({ ...generation, enable_markdown: event.target.checked })} /></label><label className="markdown-toggle"><span>Stream output</span><input type="checkbox" checked={generation.stream} onChange={(event) => onChange({ ...generation, stream: event.target.checked })} /></label></div>}</div>;
}
