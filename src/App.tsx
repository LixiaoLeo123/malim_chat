...
function ConversationView({ conversation, messages, providers, searchEnabled, busy, generation, onGenerationChange, onToggleSearch, onSend, onEdit, onDelete, onRetry, onLookup, onCompact }: { conversation: Conversation | null; messages: Message[]; providers: Provider[]; searchEnabled: boolean; busy: boolean; generation: { temperature: number; reasoning_effort: string }; onGenerationChange: (value: { temperature: number; reasoning_effort: string }) => void; onToggleSearch: () => void; onSend: (content: string) => Promise<void>; onEdit: (message: Message, content: string) => Promise<void>; onDelete: (message: Message) => Promise<void>; onRetry: (message: Message) => Promise<void>; onLookup: (word: string, anchor: { x: number; y: number }) => void; onCompact: () => Promise<void> }) {
...
function ModelConfigRow({ providerId, item, onChanged, onError }: { providerId: string; item: ProviderModel; onChanged: () => Promise<void>; onError: (value: string | null) => void }) {
  const [group, setGroup] = useState(item.group_name); const [model, setModel] = useState(item.model); const [contextWindow, setContextWindow] = useState(item.context_window);
  return <div className="model-config-row"><input value={group} aria-label="Model group" onChange={(event) => setGroup(event.target.value)} /><input value={model} aria-label="Model name" onChange={(event) => setModel(event.target.value)} /><input value={contextWindow} type="number" min="4096" max="2000000" step="1024" aria-label="Maximum context tokens" onChange={(event) => setContextWindow(Number(event.target.value))} /><button type="button" className="text-button small" onClick={async () => { try { await api.updateProviderModel(providerId, item.id, { group_name: group, model, context_window: contextWindow }); await onChanged(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Could not update model."); } }}>Save</button><button type="button" className="icon-button danger" aria-label={`Delete ${item.model}`} onClick={async () => { try { await api.deleteProviderModel(providerId, item.id); await onChanged(); } catch (cause) { onError(cause instanceof Error ? cause.message : "Could not delete model."); } }}><Trash2 size={15} /></button></div>;
}
