...
export const api = {
  ...
  createProviderModel: (providerId: string, payload: { group_name: string; model: string; kind: ProviderKind; sort_order?: number; context_window?: number; supports_images?: boolean; chain_context?: boolean }) => request<ProviderModel>(`/v1/providers/${providerId}/models`, { method: "POST", body: JSON.stringify(payload) }),
  updateProviderModel: (providerId: string, modelId: string, payload: { group_name?: string; model?: string; kind?: ProviderKind; sort_order?: number; context_window?: number; supports_images?: boolean; chain_context?: boolean }) => request<ProviderModel>(`/v1/providers/${providerId}/models/${modelId}`, { method: "PATCH", body: JSON.stringify(payload) }),
